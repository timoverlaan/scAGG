...
parser.add_argument('--slurm-job', type=int, default=None, help='SLURM job ID')
parser.add_argument("--regress", action="store_true", help="Regress out total counts and mitochondrial counts")
parser.add_argument("--scale", action="store_true", help="Scale data to unit variance and zero mean")
...
if __name__ == "__main__":
    ...
    scale_suff = "_nzscaled" if args.scale else ""
    regress_suff = "_regressed" if args.regress else ""
    knn_suff = f"_k{args.k_neighbors}"
    hvg_suff = "_s3" if args.gene_selection == "seurat_v3" else ""
    output_file = f"data/adata_rosmap_v3_top{args.n_top_genes}{hvg_suff}{knn_suff}{scale_suff}{regress_suff}.h5ad"
    ...
    plt.savefig(f"out/figures/{args.slurm_job}_qc_metrics.png")
    sc.pl.scatter(adata, x="total_counts", y="pct_counts_mt")
    plt.savefig(f"out/figures/{args.slurm_job}_mt_counts.png")
    sc.pl.scatter(adata, x="total_counts", y="n_genes_by_counts")
    plt.savefig(f"out/figures/{args.slurm_job}_n_genes.png")
    ...
    # Now drop donors with < 2.5k cells
    print("Dropping donors with < 2.5k cells...")
    donor_counts = adata.obs["Donor ID"].value_counts()
    drop_donors = donor_counts[donor_counts < 2500].keys()
    adata_drop = adata[~adata.obs["Donor ID"].isin(drop_donors), :].copy()
    del adata
    adata = adata_drop
    print(f"Dropped {len(drop_donors)} donors, new shape={adata.shape}\n")

    # And save the resulting file
    output_file = output_file.replace(".h5ad", "_drop.h5ad")
